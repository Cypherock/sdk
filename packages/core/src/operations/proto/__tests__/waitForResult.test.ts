...
import { rawWaitForResultTestCases } from '../__fixtures__/waitForResult';

describe('Raw Operations: waitForResult', () => {
  ...
  beforeEach(async () => {
    global.Date.now = jest.fn(() =>
      rawWaitForResultTestCases.constantDate.getTime(),
    );
  ...
  test('should be able to get command', async () => {
    ...
    for (const testCase of rawWaitForResultTestCases.valid) {
      statusIndex = 0;
      connection = await MockDeviceConnection.create();
      await connection.beforeOperation();
  ...
  test('should return valid errors when device returns invalid data', async () => {
    ...
    for (const testCase of rawWaitForResultTestCases.error) {
      statusIndex = 0;
      connection = await MockDeviceConnection.create();
      await connection.beforeOperation();
  ...
  test('should throw error with invalid arguments', async () => {
    for (const testCase of rawWaitForResultTestCases.invalidArgs) {
      const params = {
      ...
      await expect(waitForResult(params)).rejects.toBeInstanceOf(Error);
    }
  });
});
