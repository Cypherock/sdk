...
describe('InheritanceApp.authWallet', () => {
  ...
  describe('should be able to authenticate wallet', () => {
    fixtures.valid.forEach(testCase => {
      test(testCase.name, async () => {
        const onEvent = setupMocks(testCase);

        const output = await inheritanceApp.authWallet({
          ...testCase.params,
          onEvent,
        });
  ...
  describe('should throw error with invalid arguments', () => {
    fixtures.invalidArgs.forEach(testCase => {
      test(testCase.name, async () => {
        ...
        const rejectedPromise = inheritanceApp.authWallet(testCase.params);
