...
export const getPublicKeys = async (
  ...
  // Sia uses simple index based derivation unlike BIP
  assert(
    params.derivationPaths.reduce(
      (acc, path) => acc && path.path.length === 1,
      true,
    ),
    'derivationPaths should have exactly 1 element (index)',
  );

  assert(
    params.derivationPaths.reduce((acc, path) => {
      if (path.path.length >= 1) {
        return acc && path.path[0] >= 0;
      }
      return acc;
    }, true),
    'derivationPaths should follow Sia format: m | index',
  );
